...
fullname = "Alzheimer's disease and related dementias"
shortname = "ADRD"
...
def clinical_reasoning_prompt(
    ...
    if age is None:
        ...
        "After completing the reasoning, you MUST end the paragraph with ONE last sentence that begins exactly with: "
        "Baseline clinical interpretation: "


        "The sentence should summarize what the record is primarily about in this patient.\n\n"

        "Purpose of this sentence:\n"
        "- Do NOT center your reasoning on the absence of documented cognitive/functional assessments. "
        "The main reasoning must focus on interpreting what IS present in the baseline record and how salient each pattern is.\n"
        ...
        "Content guidelines for the final sentence:\n"
        "- Describe overarching clinical patterns or meanings inferred from the baseline record.\n"
        "- If baseline evidence does not support a coherent cognitive or functional pattern, it is acceptable to state that the record "
        "reflects nonspecific, low-salience, or diffuse systemic features grounded in what is documented.\n"
        ...
        f"Follow-up diagnoses ({year}y to 1y prior, training-only): {delta_codes_diagnosis or []}\n"
        f"Follow-up medications ({year}y to 1y prior, training-only): {delta_codes_medication or []}\n\n"
        ...
        f"Baseline demographics: age {age}.\n"
        f"Baseline diagnoses (available at prediction time): {base_codes_diagnosis or []}\n"
        f"Baseline medications (available at prediction time): {base_codes_medication or []}\n\n"
...
def calibration_update_prompt(
    ...
    return (
    f"You are a junior physician specializing in {fullname} ({shortname}) and early clinical risk interpretation. "
    f"You previously evaluated a patient at the PREDICTION TIME ({year} years before the outcome window), "
    "using baseline EHR information only, and produced a clinical reasoning paragraph interpreting baseline patterns.\n\n"

    f"A senior physician has now revealed the true outcome label indicating whether the patient developed {shortname} within the subsequent {year}-year window. "
    ...
    "Your task is to write a supervised reflection for this patient that analyzes how your prior baseline reasoning "
    ...
    "to update general experience knowledge. You are NOT asked to produce a final or global clinical rule.\n\n"

    "Important guidance:\n"
    "- Do NOT restate patient-specific diagnoses, medications, demographics, codes, or timelines.\n"
    "- Focus on how you should have adjusted the salience or weighting of certain baseline clinical patterns "
    "(e.g., cognitively anchored signals, vascular/metabolic burden, neuropsychiatric confounding, nonspecific systemic illness, other dementia characteristics).\n"
    "- Phrase your reflection in a way that could later contribute to cross-patient experience consolidation.\n\n"

    "Existing clinician knowledge (shared reference from previously seen patients; do not treat this as final or authoritative):\n"
    f"{mem}\n\n"

    "Your prior reasoning paragraph for this patient:\n"
    f"{reasoning_card}\n\n"

    f"Observed outcome label (0 = no {shortname} within {year} years, 1 = {shortname} occurred in {year} years): {int(label)}\n\n"
    ...
    "Write the supervised reflection paragraph now."
)
...
def memory_refresh_prompt(
    ...
    return (
        ...
        f"and has repeatedly calibrated your clinical judgments against true {fullname} ({shortname}) outcomes. "
